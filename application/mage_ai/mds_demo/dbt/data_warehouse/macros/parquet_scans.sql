...
-- ──────────────────────────────────────────────────────────────────────────────
-- GEE RLCMS (SERVIR-Mekong annual land-cover class area per ward)
-- Layout: s3://dwhfilesystem/landing_area/gee_rlcms/year=YYYY/*.parquet
-- Grain: one row per (year, ward, class_code)  — long format
-- Partition key: year only (annual data, no month/day dirs)
-- ──────────────────────────────────────────────────────────────────────────────
{% macro gee_rlcms_typed_scan(partition_filter='') %}
SELECT
    r['year']::INTEGER                AS year,
    r['ma_xa']::VARCHAR               AS ma_xa,
    r['ten_xa']::VARCHAR              AS ten_xa,
    r['ma_tinh']::VARCHAR             AS ma_tinh,
    r['ten_tinh']::VARCHAR            AS ten_tinh,
    r['class_code']::INTEGER          AS class_code,
    r['pixel_count']::INTEGER         AS pixel_count,
    r['area_km2']::DOUBLE PRECISION   AS area_km2,
    r['pct_area']::DOUBLE PRECISION   AS pct_area,
    r['source']::VARCHAR              AS source
FROM duckdb.query($duckdb$
    SELECT *
    FROM read_parquet(
        's3://dwhfilesystem/landing_area/gee_rlcms/**/*.parquet',
        hive_partitioning = true,
        filename          = true
    )
    {% if partition_filter %}WHERE {{ partition_filter }}{% endif %}
...
-- ──────────────────────────────────────────────────────────────────────────────
-- GEE NDVI (MODIS/061/MOD13Q1 zonal stats per Vietnamese ward)
-- Layout: s3://dwhfilesystem/landing_area/gee_ndvi/year=Y/month=M/day=D/*.parquet
-- Partition key = image_date (year/month/day of the MODIS composite).
-- ──────────────────────────────────────────────────────────────────────────────
{% macro gee_ndvi_typed_scan(partition_filter='') %}
SELECT
